...
public class RequestManagementServlet extends HttpServlet {
    ...
    private void handleListRequests(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("admin".equals(userType)) {
            ...
            // Para empleados, también obtener solicitudes disponibles
            if ("available".equals(tabParam)) {
        ...
        request.setAttribute("tab", tabParam != null ? tabParam : "assigned");
    ...
    private void takeRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Request req = requestDAO.getRequest(requestId);
            if (req == null) {
                response.sendRedirect(request.getContextPath() + "/requestManagement?error=not_found&tab=" + tabParam);
                return;
            }
            
            if (req.getAssignedTo() != null && !req.getAssignedTo().trim().isEmpty()) {
                response.sendRedirect(request.getContextPath() + "/requestManagement?error=already_assigned&tab=" + tabParam);
                return;
            }
            ...
            if (success) {
                ...
                response.sendRedirect(request.getContextPath() + "/requestManagement?success=request_taken&tab=" + tabParam);
            } else {
                response.sendRedirect(request.getContextPath() + "/requestManagement?error=take_failed&tab=" + tabParam);
            }
